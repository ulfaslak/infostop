...
class FoliumMap:
    ...
    def __init__(
        self, points, labels, zoom_start=10, tiles="OpenStreetMap", API_key=None
    ):
        self.points = points
        self.labels = labels
        self.m = folium.Map(
    ...
    def render_polygons(self, color="#ee9999", opacity=0.3):
        ...
        folium.GeoJson(features, style_function=_style_function).add_to(self.m)
    ...
    def render_points(self, color="k", opacity=0.3, subsampling=1):
        ...
        if subsampling < 1:
            mask = np.random.choice(
                range(p.shape[0]), size=int(subsampling * p.shape[0]), replace=False
            )
            p = p[mask]

        for d in p:
            folium.CircleMarker(
                d[:2], color=color, fill_color=color, fill_opacity=opacity, fill=True
            ).add_to(self.m)
...
def plot_map(
    ...
    assert model._is_fitted, "It appears that no data is fitted to the model."
    assert (
        model._distance_metric == "haversine"
    ), "`distance_metric` is not 'haversine'."
    assert display_data in [
        "all",
        "all_stationary",
        "all_nonstationary",
        "unique_stationary",
    ...
    if hasattr(model, "r1"):
        if "all" in display_data:
            data_unique, indices_unique = np.unique(
                np.vstack(model._data)[:, :2], axis=0, return_index=True
            )
    ...
    if scatter:
        folmap.render_points(
            color=scatter_color,
            opacity=scatter_opacity,
            subsampling=scatter_subsampling,
        )
    if heatmap:
