...
def compute_intervals_with_tz(coords, coord_labels, max_time_between=86400, distance_metric="haversine"):
    ...
    #initialize values
    lat_prec, lon_prec, t_start, tz, loc_prec = trajectory[0]  
    t_end = t_start 
    median_lat = [lat_prec]
    median_lon = [lon_prec]

    #Loop through trajectory
    for lat, lon, time, tz, loc in trajectory[1:]:
        ...
        if (loc==loc_prec) and (time-t_end)<(max_time_between):
            ...
            if loc_prec==-1:
                final_trajectory.append([loc_prec, t_start,  t_end, tz, np.nan, np.nan])
            else:
                final_trajectory.append([loc_prec, t_start,  t_end, tz, np.median(median_lat), np.median(median_lon)])
